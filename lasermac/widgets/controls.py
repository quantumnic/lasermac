...
class ControlsPanel(ctk.CTkFrame):
    ...
    def __init__(self, parent, grbl: GrblController, **kwargs) -> None:
        ...
        btn_size = 50
        btn_cfg = {"width": btn_size, "height": btn_size, "fg_color": "#333333", "hover_color": "#444444"}

        # Y+
        ctk.CTkButton(
            jog_frame, text="▲\nY+", command=lambda: self._jog("Y", 1), **btn_cfg
        ).grid(row=0, column=1, padx=2, pady=2)

        # X-
        ctk.CTkButton(
            jog_frame, text="◀\nX-", command=lambda: self._jog("X", -1), **btn_cfg
        ).grid(row=1, column=0, padx=2, pady=2)

        # Home icon
        ctk.CTkButton(
            jog_frame, text="⌂", command=lambda: self.grbl.go_to_origin(),
            width=btn_size, height=btn_size, fg_color="#1a5276", hover_color="#2471a3",
        ...
        ctk.CTkButton(
            z_frame, text="Z+", command=lambda: self._jog("Z", 1), **btn_cfg
        ).pack(pady=2)
        ctk.CTkButton(
            z_frame, text="Z-", command=lambda: self._jog("Z", -1), **btn_cfg
        ).pack(pady=2)
        ...
        self.pos_label = ctk.CTkLabel(
            self, text="X: 0.000  Y: 0.000  Z: 0.000", font=("Menlo", 13)
        )
        ...
        for text, cmd in actions:
            ctk.CTkButton(
                action_frame, text=text, command=cmd, width=90,
